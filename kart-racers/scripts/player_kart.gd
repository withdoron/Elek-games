...
var drift_charge: int = 0  # 0=none, 1=white, 2=yellow, 3=orange (ready)
var drift_charge_timer: float = 0.0
var drift_direction: float = 0.0
var drift_boost_timer: float = 0.0  # time remaining on boost after release
...
func _physics_process(delta: float) -> void:
	...
	# Oil drop = Y button (or E key for P1)
	...
	if oil_button and oil_count > 0 and on_ground:
		oil_count -= 1
		var drop_pos = global_position + transform.basis.z * 3.0  # drop behind truck
		emit_signal("drop_oil", player_id, drop_pos)
		# Debounce — set flag so we don't drop multiple per press
		# (handled by oil_count decrement)

	# --- MK64 Powerslide ---
	if drift_button and on_ground and abs(speed) > 8.0:
		if not is_drifting:
			# Start drift — small hop
			is_drifting = true
			...
			# Continue drifting — charge the boost by steering back and forth
			drift_charge_timer += delta
			if drift_charge_timer > 0.4 and drift_charge < 2:
				drift_charge = 2  # yellow
			if drift_charge_timer > 0.8 and drift_charge < 3:
				drift_charge = 3  # orange — full charge
	elif is_drifting and not drift_button:
		# Release drift — apply boost based on charge level
		is_drifting = false
		if drift_charge >= 2:
	...
	var surface_max = s.max_speed if on_road else s.max_speed * 0.85

	# Drift boost adds to max speed temporarily
	if drift_boost_timer > 0:
		drift_boost_timer -= delta
		surface_max += DRIFT_BOOST_SPEED

	if throttle > 0.0 and speed >= 0.0:
		speed = move_toward(speed, surface_max * throttle, s.acceleration * throttle * delta)
	...
	if abs(speed) > 0.5:
		...
		if is_drifting:
			# During drift: sharper turning, biased toward drift direction
	...
	if is_drifting:
		# Drifting: high lateral persistence — truck slides sideways
		move_velocity = forward_vel + lateral * 0.96
	else:
		# Normal: grip decreases at high speed
...
func _update_audio(on_road: bool) -> void:
	...
	engine_audio.volume_db = -10.0 + spd_ratio * 8.0
	# Drift boost: higher pitch
	if drift_boost_timer > 0:
		engine_audio.pitch_scale += 0.3
...
func _setup_drift_sparks() -> void:
	...
	drift_sparks.color = Color(1.0, 0.8, 0.2)  # starts white/yellow
...
func _update_visuals(delta: float, steer_input: float, on_road: bool) -> void:
	...
	for p in dirt_particles:
		if spd_ratio > 0.1 and on_ground:
			...
			p.color = Color(0.45, 0.35, 0.22, 0.8) if on_road else Color(0.3, 0.55, 0.2, 0.8)
		else:
			p.emitting = false

	# Drift sparks — color shows charge level
	if drift_sparks:
		if is_drifting and on_ground:
			drift_sparks.emitting = true
			if drift_charge >= 3:
				drift_sparks.color = Color(1.0, 0.4, 0.1)  # orange = full charge
			elif drift_charge >= 2:
				drift_sparks.color = Color(1.0, 0.9, 0.2)  # yellow = half charge
			else:
				drift_sparks.color = Color(0.8, 0.8, 0.8)  # white = just started
		else:
			drift_sparks.emitting = false
...
func _build_truck() -> void:
	...
	(windshield.material_override as StandardMaterial3D).transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	...
	(rear_window.material_override as StandardMaterial3D).transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	...
	_make_bumper(Vector3(0, 1.1, -1.7), chrome)
	_make_bumper(Vector3(0, 1.1, 1.85), chrome)
	...
	for x_pos in [-0.6, 0.6]:
		...
		hl_mat.emission = Color(1.0, 0.9, 0.3)
		hl_mat.emission_energy_multiplier = 2.0
		body_mesh.add_child(hl)

	var wheel_positions = [
		Vector3(-1.4, 0.8, -1.1), Vector3(1.4, 0.8, -1.1),
		Vector3(-1.4, 0.8, 1.1), Vector3(1.4, 0.8, 1.1),
	]
	...
	for x_pos in [-0.5, 0.5]:
		var exhaust = _make_cylinder(0.06, 0.5, Color(0.2, 0.2, 0.22))
		exhaust.position = Vector3(x_pos, 1.8, 1.5)
		body_mesh.add_child(exhaust)


func _make_bumper(pos: Vector3, color: Color) -> void:
	var b = _make_box_metallic(Vector3(2.2, 0.2, 0.15), color, 0.7)
	b.position = pos
	body_mesh.add_child(b)
...
func _make_monster_wheel() -> Node3D:
	...
	tc.top_radius = 0.8
	tc.bottom_radius = 0.8
	tc.height = 0.6
	tire.mesh = tc
	tire.rotation_degrees = Vector3(0, 0, 90)
	var tm = StandardMaterial3D.new()
	tm.albedo_color = Color(0.1, 0.1, 0.1)
	tm.roughness = 1.0
